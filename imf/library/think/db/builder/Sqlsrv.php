...
{
    protected $selectSql       = 'SELECT T1.* FROM (SELECT imf.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS imf) AS T1 %LIMIT%%COMMENT%';
