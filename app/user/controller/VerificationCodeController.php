...
class VerificationCodeController extends HomeBaseController
{
    public function send()
    {
        ...
        $code = imf_get_verification_code($data['username']);
        if (empty($code)) {
        ...
        if ($accountType == 'email') {
            ...
            $emailTemplate = imf_get_option('email_template_verification_code');
            ...
            $subject = empty($emailTemplate['subject']) ? 'ThinkIMF验证码' : $emailTemplate['subject'];
            $result  = imf_send_email($data['username'], $subject, $message);

            if (empty($result['error'])) {
                imf_verification_code_log($data['username'], $code);
