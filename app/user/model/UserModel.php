...
class UserModel extends Model
{
    public function doMobile($user)
    {
        ...
        if (!empty($result)) {
            $comparePasswordResult = imf_compare_password($user['user_pass'], $result['user_pass']);
            $hookParam =[
                'user'=>$user,
    ...
    public function registerEmail($user)
    {
        ...
        if (empty($result)) {
            $data   = [
                ...
                'mobile'          => '',
                'user_nickname'   => '',
                'user_pass'       => imf_password($user['user_pass']),
                'last_login_ip'   => get_client_ip(0, true),
                'create_time'     => time(),
                'last_login_time' => time(),
    ...
    public function emailPasswordReset($email, $password)
    {
        ...
        if (!empty($result)) {
            $data = [
                'user_pass' => imf_password($password),
            ];
    ...
    public function editData($user)
    {
        ...
        if ($userQuery->where('id', $userId)->update($data)) {
            ...
            imf_update_current_user($userInfo);
            return 1;
        }
        return 0;
    }
    ...
    public function editPassword($user)
    {
        ...
        if (!imf_compare_password($user['old_password'], $pass['user_pass'])) {
            return 2;
        }
        $data['user_pass'] = imf_password($user['password']);
