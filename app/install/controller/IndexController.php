...
class IndexController extends Controller
{
    ...
    {
        if (imf_is_installed()) {
            $this->error('网站已经安装', imf_get_root() . '/');
        }
    }
...
//        $safe_mode          = (ini_get('safe_mode') ? '<font color=red>[×]On</font>' : '<font color=green>[√]Off</font>');
        ...
        if (version_compare(phpversion(), '7.0.0', '>=') && version_compare(phpversion(), '7.0.0', '<') && ini_get('always_populate_raw_post_data') != -1) {
    ...
    public function step4()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $sql = imf_split_sql(APP_PATH . 'install/data/thinkimf.sql', $dbConfig['prefix'], $dbConfig['charset']);
    ...
    public function setDbConfig()
    {
        ...
        $dbConfig['authcode'] = imf_random_string(18);
    ...
    public function setSite()
    {
        ...
        $admin['id']            = 1;
        $admin['user_pass']     = imf_password($admin['user_pass']);
        $admin['user_type']     = 1;
        ...
        try {
            imf_set_option('site_info', $siteInfo);
