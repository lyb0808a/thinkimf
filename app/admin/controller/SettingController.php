...
class SettingController extends AdminBaseController
{
    ...
    public function site()
    {
        ...
        $adminStyles    = imf_scan_dir($adminThemesDir . '*', GLOB_ONLYDIR);
        ...
        $this->assign(imf_get_option('site_info'));
    ...
    public function sitePost()
    {
        if ($this->request->isPost()) {
            ...
            imf_set_option('site_info', $options);
            ...
            imf_set_option('cmf_settings', $cmfSettings);

            $cdnSettings = $this->request->param('cdn_settings/a');
            imf_set_option('cdn_settings', $cdnSettings);
            ...
            imf_set_option('admin_settings', $adminSettings);
    ...
    public function passwordPost()
    {
        if ($this->request->isPost()) {
            ...
            $userId = imf_get_current_admin_id();
            ...
            if (imf_compare_password($oldPassword, $admin['user_pass'])) {
                if ($password == $rePassword) {

                    if (imf_compare_password($password, $admin['user_pass'])) {
                        $this->error("新密码不能和原始密码相同！");
                    } else {
                        Db::name('user')->where('id', $userId)->update(['user_pass' => imf_password($password)]);
                        $this->success("密码修改成功！");
                    }
                } else {
                    $this->error("密码输入不一致！");
    ...
    public function uploadPost()
    {
        if ($this->request->isPost()) {
            ...
            imf_set_option('upload_setting', $uploadSetting);
            $this->success('保存成功！');
        }
